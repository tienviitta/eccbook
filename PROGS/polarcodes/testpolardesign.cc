...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < N; i++)
        savepolarBhatt[i] = polarcode.polarcodedesign[i];
    ...
    for (int i = 0; i < N; i++) {
        if (savepolarBhatt[i] != polarcode.polarcodedesign[i]) {
            cout << "designs different at i=" << i << "  Bhatt=" << int(savepolarBhatt[i])
                 << "  MC=" << int(polarcode.polarcodedesign[i]) << endl;
        }
    }
    exit(0);
}
...
/*
Local Variables:
compile-command: "clang++ -o testpolardesign -g testpolardesign.cc polarcode.cc
quicksortstuff.cc randstuff.cc -std=c++11 -stdlib=libc++ -lm;" End:
*/
